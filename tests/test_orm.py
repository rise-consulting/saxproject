...
def test_create_tables(db, Author, Book):
    ...
    ) == "CREATE TABLE author (id INTEGER PRIMARY KEY AUTOINCREMENT, age INTEGER, name TEXT);"
    assert Book._get_create_sql(
    ) == "CREATE TABLE book (id INTEGER PRIMARY KEY AUTOINCREMENT, author_id INTEGER, published INTEGER, title TEXT);"

    for table in ("author", "book"):
        assert table in db.tables


def test_create_author_instance(db, Author, Book):
    db.create(Author)
    db.create(Book)
...
def test_get_book(db, Author, Book):
    ...
    author_from_db = db.get(Author, 2)

    assert author_from_db.id == 2
